...
@implementation EXDataGridColumn
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    EXDataGridCell *cell = [_dataGridDelegate dataGrid:(id)self.superview cellForColumn:_columnNumber row:(int)indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if([_dataGridDelegate respondsToSelector:@selector(dataGrid:didSelectCellAtColumn:row:)]) {
        [_dataGridDelegate dataGrid:(id)self.superview didSelectCellAtColumn:_columnNumber row:(int)indexPath.row];
    }
}

- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    cellHeight = [_dataGridDelegate dataGrid:(id)self.superview heightOfRow:(int)indexPath.row];
    return cellHeight;
}
...
- (UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if(!view) {
        ...
        if([_dataGridDelegate respondsToSelector:@selector(titleForColumn:)]) {
            ...
            if(title) {
                EXHeaderView *headerView = [[[EXHeaderView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, headerViewHeight)] autorelease];
...
- (void)longPressGesture:(UILongPressGestureRecognizer*)longPressGesture
{
    ...
    if(longPressGesture.state == UIGestureRecognizerStateBegan) {
        ...
        for(UIView *view in self.subviews) {
            if([view isMemberOfClass:[EXHeaderView class]]) {
                [view setAlpha:0.98];
            }
            [view setAlpha:0.8];
        }
